...
async function fetchOdds(sportKey, marketTypes) {
    ...
    if (!apiKey)
        return ['⚠️ No Odds API key found. Set ODDS_API_KEY in config.json'];
...
async function getDailyScheduledTips(limit = 3) {
    ...
    for (const league of shuffledLeagues) {
        ...
        if (league.includes('la_liga'))
            leagueName = '🇪🇸 La Liga';
        else if (league.includes('serie_a'))
            leagueName = '🇮🇹 Serie A';
        ...
        for (const game of shuffledEvents.slice(0, 1)) { // Take 1 random match per league
            ...
            allTips.push(`${leagueName}\n⚽ ${home} vs ${away}\n• Start: ${commence}\n• Odds — Home: ${homeOdds} | Draw: ${drawOdds} | Away: ${awayOdds}\n• 🧮 Win Chances — ${home}: ${impliedProbability(homeOdds)} | ${away}: ${impliedProbability(awayOdds)}`);
        }
        if (allTips.length >= limit)
            break;
    }
